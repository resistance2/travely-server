...
travelRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const newTravel = await Travel.findById(travelId)
        .populate("teamId")
        .lean();
...
travelRouter.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const travels = await Travel.find({ userId: user._id })
        .populate("teamId")
        .lean();
      ...
      const userTravelDetails = await Promise.all(
        travels.map(async (travel) => {
          ...
          return {
            ...
            travelReviewCount: populatedTravel?._id
              ? await getReviewCount(populatedTravel._id)
              : 0,
            travelActive: populatedTravel?.travelActive,
            updatedAt: populatedTravel?.updatedAt,
            reviewAverage: populatedTravel?._id
              ? await getReviewAverage(populatedTravel._id)
              : 0,
            approveWaitngCount: (Array.isArray(populatedTravel?.teamId)
              ...
              return (
                acc +
                team.appliedUsers.filter(
                  (user: { status: string }) => user.status === "waiting"
                ).length
              );
            }, 0),
          };
    ...
    try {
      ...
      const teams = await Team.findOne({ travelId, _id: teamId })
        .populate({
          path: "appliedUsers.userId",
          select:
            "userProfileImage socialName userName userEmail phoneNumber mbti",
        })
        .lean();
...
travelRouter.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const teams = (await Team.find({ travelId }).select("_id")).map(
        (team) => team._id
      );
      res.json(
        ResponseDTO.success({
